...
function CityList({ cityList, onDeleteCity }) {
  return (
    ...
    <div>
      <h2 className='w-36 h-10 mt-2 mb-2 bg-blue-500 text-white font-semibold rounded-lg border border-blue-500 hover:bg-blue-700 font-bold focus:bg-blue-700 cursor-pointer flex items-center justify-center'>Lista de Ciudades</h2>
      <ul>
        {cityList.map((city, index) => (
          <li key={index} className="bg-white bg-opacity-10 backdrop-filter backdrop-blur-sm p-4 rounded-md shadow-xl mb-2 flex flex-col w-1/4 h-1/2">
            <div>
